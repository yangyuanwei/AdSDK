 ...
 *  @param userId 用户id
 */
+ (NSData *)bindDataWithUserId:(NSString *)userId andIsUnbindFlag:(BOOL)isUnbindFlag Tag:(NSString *)tag{
    //body数据包
    NSMutableData *bodyData = [NSMutableData data];
    RFIWriter *writerPacket = [RFIWriter writerWithData:bodyData];
...
+ (id)processRecievePushMessageWithPacket:(IP_PACKET)packet andData:(NSData *)body_data{
    ...
    //    NSDictionary *dict = contentDic[@"clickActContent"];
